...
def add_sample(authorID, authorNameRaw, commentText):
    ...
    matchSamplesDict[authorID] = {'index'      : index, 'cString': cString, 'iString': iString, 'count': authorNumComments, 'authorID': authorID, 'authorName': authorNameRaw,
...
def get_comments(youtube, miscData, currentUser, filterMode, filterSubMode, scanVideoID=None, check_channel_id=None, nextPageToken=None, inputtedSpammerChannelID=None, inputtedUsernameFilter=None,
                 inputtedCommentTextFilter=None, regexPattern=None):  # None are set as default if no parameters passed into function
    ...
    for item in results["items"]:
        ...
        check_against_filter(currentUser, miscData, filterMode=filterMode, filterSubMode=filterSubMode, commentID=parent_id, videoID=videoID, authorChannelID=parentAuthorChannelID,
                             parentAuthorChannelID=None, inputtedSpammerChannelID=inputtedSpammerChannelID, inputtedUsernameFilter=inputtedUsernameFilter,
                             inputtedCommentTextFilter=inputtedCommentTextFilter, authorChannelName=authorChannelName, commentText=commentText, regexPattern=regexPattern)
        ...
        if numReplies > 0 and len(limitedRepliesList) < numReplies:
            get_replies(currentUser, miscData, filterMode, filterSubMode, parent_id, videoID, parentAuthorChannelID, inputtedSpammerChannelID, inputtedUsernameFilter, inputtedCommentTextFilter,
                        regexPattern)
        elif numReplies > 0 and len(limitedRepliesList) == numReplies:  # limitedRepliesList can never be more than numReplies
            get_replies(currentUser, miscData, filterMode, filterSubMode, parent_id, videoID, parentAuthorChannelID, inputtedSpammerChannelID, inputtedUsernameFilter, inputtedCommentTextFilter,
                        regexPattern, limitedRepliesList)
        else:
...
def get_replies(currentUser, miscData, filterMode, filterSubMode, parent_id, videoID, parentAuthorChannelID, inputtedSpammerChannelID=None, inputtedUsernameFilter=None, inputtedCommentTextFilter=None,
                regexPattern=None, repliesList=None):
    ...
    for reply in replies:
        ...
        # Runs check against comment info for whichever filter data is relevant
        check_against_filter(currentUser, miscData, filterMode=filterMode, filterSubMode=filterSubMode, commentID=replyID, videoID=videoID, authorChannelID=authorChannelID,
                             parentAuthorChannelID=parentAuthorChannelID, inputtedSpammerChannelID=inputtedSpammerChannelID, inputtedUsernameFilter=inputtedUsernameFilter,
                             inputtedCommentTextFilter=inputtedCommentTextFilter, authorChannelName=authorChannelName, commentText=commentText, regexPattern=regexPattern,
...
def check_against_filter(currentUser, miscData, filterMode, filterSubMode, commentID, videoID, authorChannelID, parentAuthorChannelID=None, inputtedSpammerChannelID=None, inputtedUsernameFilter=None,
                         inputtedCommentTextFilter=None, authorChannelName=None, commentText=None, regexPattern=None, allThreadAuthorNames=None):
    global vidIdDict
    global matchedCommentsDict
    commentTextOriginal = str(commentText)

    debugSingleComment = False
    if debugSingleComment:
    ...
    if currentUser[0] != authorChannelID and miscData['channelOwnerID'] != authorChannelID:
        ...
        ## Also Check if reply author ID is same as parent comment author ID, if so, ignore (to account for users who reply to spammers)
            ...
            if debugSingleComment:
                if input("Sensitive True/False: ").lower() == 'true':
                    sensitive = True
                else:
                    sensitive = False
            ...
            # Multi Criteria Tests
                ...
                if hrtTest >= 2:
                    ...
                    if sensitive:
                        redCount += 1
                elif hrtTest >= 1 and sensitive:
                    yellowCount += 1

                if yellowAdEmojiSet.intersection(combinedSet):
                    yellowCount += 1

                if spamGenEmojiSet.intersection(combinedSet) and not sensitive:
                    yellowCount += 1
...
def get_current_user(config):
    ...
    try:
        ...
        try:
            ...
            print(
                "Error Getting Current User: Channel ID was found, but channel title was not retrieved. " +
                "If this occurs again, try deleting 'token.pickle' file and re-running. If that doesn't work, " +
                "consider filing a bug report on the GitHub project 'issues' page.")
    ...
    if config is None:
        ...
        configMatch = None
    elif validate_channel_id(config['your_channel_id'])[0]:
...
def print_count_stats(final):
    if final:
        print(f"Top Level Comments Scanned: {F.YELLOW}" + str(scannedCommentsCount) + f"{S.R} | Replies Scanned: {F.YELLOW}" + str(
            scannedRepliesCount) + f"{S.R} | Matches Found So Far: {F.LIGHTRED_EX}" + str(len(matchedCommentsDict)) + f"{S.R}\n")
    else:
        print(f"Top Level Comments Scanned: {F.YELLOW}" + str(scannedCommentsCount) + f"{S.R} | Replies Scanned: {F.YELLOW}" + str(
            scannedRepliesCount) + f"{S.R} | Matches Found So Far: {F.LIGHTRED_EX}" + str(len(matchedCommentsDict)) + f"{S.R}", end="\r")
...
# Note: Most uses of this function are obsolete after changing it so the program totally ignores current user's own comments
    ...
    if not bypass:
        if filterCharsSet:
            if any(x in filterCharsSet for x in currentUsernameChars):
                print(f"\n{B.LIGHTRED_EX}{F.BLACK}NOTE!{S.R} Character(s) you entered are within {F.LIGHTRED_EX}your own username{S.R}, ' " + currentUserName + " '! : " + str(
                    filterCharsSet & currentUsernameChars))
    ...
    while not validFile and not manuallyEnter:
        ...
        if len(recoveryFileName) > 0:
            ...
            if os.path.exists(recoveryFileName):
                ...
                if choice("Try entering file name again?"):
                    pass
                else:
                    manuallyEnter = True
        else:
...
def prepare_filter_mode_chars(currentUser, scanMode, filterMode, config):
    ...
    while not validEntry:
        ...
        if validEntry:
            ...
            if choice("Begin Scanning? ", bypass):
                validEntry = True
            else:
                validEntry = False
...
def prepare_filter_mode_non_ascii(currentUser, scanMode, config):
    ...
    while not confirmation:
        ...
        if selection == "1":
            ...
            if choice("Choose this mode?", bypass):
...
def main():
    ...
    if config is not None:
        if config['use_this_config'] == 'ask':
            ...
            if not choice(f"\nFound {F.YELLOW}config file{S.R}, use those settings?"):
                config = None
            os.system(clear_command)
        elif not config['use_this_config']:
            config = None
        elif config['use_this_config']:
            pass
        else:
    ...
    while not confirmedCorrectLogin:
        ...
        if choice("       Continue as this user?", currentUser[2]):
    ...
    if scanMode == "chosenVideos":
        ...
        while validVideoIDResult[0] == False or confirm == False:
            ...
            if validVideoIDResult[0]:  # validVideoID now contains True/False and video ID
        ...
        while validEntry == False or confirm == False:
            ...
            if validEntry:
                ...
                if config and (config['skip_confirm_video'] or validConfigSetting):
                    confirm = True
                else:
    ...
    try:
        ...
        def scan_video(youtube, miscData, currentUser, filterMode, filterSubMode, videoID, check_channel_id, inputtedSpammerChannelID, inputtedUsernameFilter, inputtedCommentTextFilter, regexPattern,
                       videoTitle=None, showTitle=False, i=1):
            nextPageToken = get_comments(youtube, miscData, currentUser, filterMode, filterSubMode, videoID, check_channel_id, inputtedSpammerChannelID=inputtedSpammerChannelID,
                                         inputtedUsernameFilter=inputtedUsernameFilter, inputtedCommentTextFilter=inputtedCommentTextFilter, regexPattern=regexPattern)
            if showTitle and videosToScan:
            ...
            while nextPageToken != "End" and scannedCommentsCount < maxScanNumber:
                nextPageToken = get_comments(youtube, miscData, currentUser, filterMode, filterSubMode, videoID, check_channel_id, nextPageToken, inputtedSpammerChannelID=inputtedSpammerChannelID,
                                             inputtedUsernameFilter=inputtedUsernameFilter, inputtedCommentTextFilter=inputtedCommentTextFilter, regexPattern=regexPattern)


        if scanMode == "entireChannel":
            scan_video(youtube, miscData, currentUser, filterMode, filterSubMode, scanVideoID, check_channel_id, inputtedSpammerChannelID, inputtedUsernameFilter, inputtedCommentTextFilter,
                       regexPattern)
            ...
            for video in videosToScan:
                ...
                scan_video(youtube, miscData, currentUser, filterMode, filterSubMode, videoID, check_channel_id, inputtedSpammerChannelID, inputtedUsernameFilter, inputtedCommentTextFilter,
                           regexPattern, videoTitle=videoTitle, showTitle=True, i=i)
                i += 1
        ...
        if confirmDelete is None:
            ...
            while confirmDelete != "DELETE" and confirmDelete != "REPORT" and confirmDelete != "HOLD":
                ...
                if not exclude:
                    print(
                        f" > To {F.LIGHTGREEN_EX}exclude certain authors{S.R}: Type \'{F.LIGHTGREEN_EX}exclude{S.R}\' " +
                        f"followed by a list of the numbers {F.LIGHTMAGENTA_EX}in the sample list{S.R} next to those authors")
                    print("      > Example:  exclude 1, 12, 9")

                # Delete Instructions
                if not exclude:
                    if not userNotChannelOwner:
                ...
                confirmDelete = input("\nInput: ")
                if confirmDelete == "DELETE" and not userNotChannelOwner:
                    deletionEnabled = True
                    deletionMode = "rejected"
                elif confirmDelete == "HOLD" and (not userNotChannelOwner or moderator_mode):
                    deletionEnabled = True
                    deletionMode = "heldForReview"
